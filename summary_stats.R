...
## Community ##
sync_summary<-loreau_com_complete %>%
  group_by(Treatment) %>%
...
# for power analysis
sync_summary_all<-loreau_com_complete %>%
  #group_by(Treatment) %>%
  summarize_at(vars(synchrony),funs(mean,sd)) %>%
  mutate_if(is.numeric, format, 1)


sync_means <- pull(sync_summary, mean)

sync_both_sd<-as.vector(sync_summary_all[1,2])

sync_ES<-(as.numeric(sync_means[1])-as.numeric(sync_means[2])/as.numeric(sync_both_sd))




## Metacommunity ##
sync_meta_summary<-loreau_metacom_complete %>% 
...
# for power analysis
sync_m_means <- pull(sync_meta_summary, mean)
